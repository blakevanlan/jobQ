      ...
      this.redis.multi()
         ...
         .exec((function (err, replies) {
            ...
            if (replies[replies.length - 1] == 0) {
               ...
               // Run the job then move to the next job.
               this._runJob(job, index, jobDef, (function () {
                   this._nextJob(0);
               }).bind(this));
            }
         }).bind(this))
...
// Run the actual job.
Worker.prototype._runJob = function (job, index, jobDef, cb) {
...
// Unlock the job
Worker.prototype._unlockJob = function (job, cb) {
   if (job.lockingId) {
      // Release lock
      this.redis.del('lock:' + job.lockingId, (function (err) {
         if (err) {
            this.emit('error', 'Failed when attempting to unlock', err);
            this.state = 'error';
         } else {
            cb();
         }
